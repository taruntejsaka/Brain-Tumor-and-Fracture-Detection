...
if 'user' not in st.session_state or st.session_state.user is None:
    ...
    # Navigation menu in sidebar - Show Reports tab only for Doctors
    if user['role'] == "Doctor":
        page = st.sidebar.radio("Navigation", ["Diagnosis", "Reports", "Export Data"])
    else:
        page = st.sidebar.radio("Navigation", ["Diagnosis", "Export Data"])
            ...
            if uploaded_tumor_image is not None:
                ...
                if st.session_state.diagnosis_result and user['role'] == "Doctor" and not st.session_state.report_saved:
                    ...
                    if not patients:
                        ...
                        save_button = st.button("Save Patient Report", key="save_tumor_report_btn")
                        if save_button:
            ...
            if uploaded_fracture_image is not None:
                ...
                if st.session_state.diagnosis_result and user['role'] == "Doctor" and not st.session_state.report_saved:
                    ...
                    if not patients:
                        ...
                        save_button = st.button("Save Patient Report", key="save_fracture_report_btn")
                        if save_button:
                            patient_id = patient_options[selected_patient]
                            doctor_id = user['id']
                            diagnosis_type = st.session_state.diagnosis_type
    ...
    elif page == "Reports" and user['role'] == "Doctor":
        ...
        if not reports:
            ...
            for patient_name, patient_reports_list in patient_reports.items():
                with st.expander(f"Reports for {patient_name}"):
                    for report in patient_reports_list:
                        st.write("---")
                        ...
                        with col2:
                            if report['image_path'] and os.path.exists(report['image_path']):
                                st.image(report['image_path'], caption=f"Diagnostic Image", use_container_width=True)
                            else:
                                st.warning(f"Image not found: {report['image_path']}")
                        
                        # Generate PDF report
                        if st.button(f"Generate PDF Report", key=f"pdf_{report['id']}"):
            ...
            if export_format == "CSV":
                ...
                if st.button("Generate PDF Report"):
                    ...
                    for report in reports:
                        ...
                        if report['notes']:
                            ...
                            all_reports_pdf.set_font('Arial', '', 12)
                            all_reports_pdf.multi_cell(0, 10, report['notes'])
                        
                        if report['image_path'] and os.path.exists(report['image_path']):
                            all_reports_pdf.image(report['image_path'], x=10, y=None, w=180)
                ...
                if st.button("Generate JSON Export"):
                    ...
                    for report in reports:
                        json_data.append({
                            "id": report['id'],
                            "related_user": report['related_user'],
                            "date": report['report_date'],
                            "diagnosis_type": report['diagnosis_type'],
                            "result": report['diagnosis_result'],
                            "notes": report['notes']
                        })
